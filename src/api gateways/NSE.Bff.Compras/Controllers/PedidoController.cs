...
namespace NSE.Bff.Compras.Controllers
{
    ...
    public class PedidoController : MainController
    {
        ...
        [HttpGet]
        [Route("compras/pedido/ultimo")]
        public async Task<IActionResult> UltimoPedido()
        {
            var pedido = await _pedidoService.ObterUltimoPedido();
            if(pedido is null)
            {
                AdicionarErroProcessamento("Pedido nao encontrado");
        ...
        [HttpGet]
        [Route("compras/pedido/lista-cliente")]
        ...
        private async Task<bool> ValidarCarrinhoProdutos(CarrinhoDTO carrinho, IEnumerable<ItemProdutoDTO> produtos)
        {
            if (carrinho.Itens.Count != produtos.Count())
            {
                ...
                foreach (var itemId in itensIndisponiveis)
                {
                    ...
                    AdicionarErroProcessamento($"O item {itemCarrinho.Nome} nao esta mais disponivel no catalogo, o remova do carrinho do carrinho para prosseguir com a compra");
                }
            ...
            foreach (var itemCarrinho in carrinho.Itens)
            {
                ...
                if(produtoCatalogo.Valor != itemCarrinho.Valor)
                {
                    ...
                    {
                        AdicionarErroProcessamento($"Nao foi possivel atualizar automaticamente o produto {itemCarrinho.Nome} do seu carrinho, _" +
                                                   "remove e adicione novamente caso ainda deseje comprar este item");
                        return false;
                    }
                    ...
                    AdicionarErroProcessamento(msgErro + "Atualizamos o valor em seu carrinho, realize a conferencia do pedido e se preferir remova o produto");
        ...
        private void PopularDadosPedido(CarrinhoDTO carrinho, EnderecoDTO endereco, PedidoDTO pedido)
        {
            ...
            pedido.PedidosItems = carrinho.Itens;
